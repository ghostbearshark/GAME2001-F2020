...
public:
	...
	//Remove and item given an index
	...
	void SetGrowSize(int val) //Growing by whatever the grow size is in the construtor.
		//in assignment, I have to grow my different values each time.
	{
		assert(val >= 0);
		m_growSize = val;
	}
...
private:
	//Variables
	T* m_array;  //pointer to the beginnging of the array
