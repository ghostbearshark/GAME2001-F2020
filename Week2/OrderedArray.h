...
public:
	...
	~OrderedArray()
	{
		...
		{
			delete[] m_array;
			m_array = NULL; // Good programming prcatice to set unused pointers to NULL
		}
	}
	...
	int search(T searchKey)
	{
		...
		while (1) // <-- This will be replaced with recursion
		{
			...
			{
				//Found the item in the middile of the array. Return the index
				return current;
			}
			//Step 2: Check that we've exhaused all options already. Can't find the item
...
private:
	...
	int m_maxSize; // max number of items the array can hold
